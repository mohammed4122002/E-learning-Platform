...
| T20 | TG icon names vs drawn glyphs (trainer courses) | Several TG icon instances are named after one Lucide icon but draw another (e.g. «icon · banknote»/«link»/«pause» draw an hourglass, «monitor-play» draws «tv», «layers» draws «puzzle», «play» draws «calendar-check», «message-square» draws «messages-square», «folder» draws «bookmark»). | The trainer-course screens now render the glyph Figma actually draws. Confirm whether the library names or the drawn glyphs are intended. |
