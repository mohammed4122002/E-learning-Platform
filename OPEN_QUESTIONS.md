...
## Trainer finance (Wave 2) — design decisions needed

| # | Screen (Figma) | Question | Current behaviour |
| --- | --- | --- | --- |
| W2-FIN-1 | TRR-FIN-01/02 release & settlement | Figma's «دورة التسوية» lists «أُغلقت التسوية» before «متاحة للسحب», but FIN-01 also releases each amount 7 days after the course ends (14 days after each recorded purchase). Must money wait for the monthly close? | Amounts become available on their release date; a settlement is the calendar month (Asia/Riyadh) of release, numbered STL-YYYY-MMDD (month end). The current month's settlement reads «تُغلق …» instead of «أُغلقت …» until the month ends. |
| W2-FIN-2 | TRR-FIN-02 refunds (301:8728 vs 310:10328) | 301:8728 says a withdrawal before the start is refunded «لا خصم عليك»; 310:10328 deducts the refund and a 3 % processing fee. | Follows 310:10328: the refunded share (commission returned pro rata) is deducted, plus `refund_processing_fee_percent` (3 %, app_settings) when a trainee leaves a scheduled course before it starts. |
| W2-FIN-3 | TRR-FIN-02 totals «تسويات معلّقة من فبراير» | No carry-over concept is defined. | Each settlement stands alone; the row is not shown. |
| W2-FIN-4 | TRR-FIN-01/02/04 sample numbers | Figma numbers do not reconcile (e.g. «عمولة المنصة ١٠٪» next to 12 % amounts, «المبلغ المحوَّل» equal to the requested amount despite the 15 ر.س fee). | Real ledger figures; «المبلغ المحوَّل» shows the amount after the transfer fee. |
| W2-FIN-5 | TRR-FIN-03 (301:9016) | No design for a trainer with no account yet, for a change awaiting verification, or for a rejected account. Figma also draws the tabs at the bottom of this frame. | Built with the same card styles: «أضف حسابك البنكي» (form only), a warning-tone «حسابك (الجديد) قيد التحقق» card with «ألغِ التغيير» (within 48 h). Tabs sit at the top like the other finance frames. The account holder is the profile name (no holder field in Figma). |
| W2-FIN-6 | TRR-FIN-03 verification | There is no admin screen. | An admin verifies with `admin_review_bank_account(account, approve, note)`; withdrawals open 48 h after the account was submitted. |
| W2-FIN-7 | TRR-FIN-04 payouts | There is no payout provider. | A request stays «قيد المعالجة» until an admin calls `admin_update_withdrawal(id, 'processing'/'completed'/'failed', reason, transfer_ref)`; only «completed» reduces the balance. One request at a time. The failure reason is added to the failed panel and to «طلبات سابقة». |
| W2-FIN-8 | TRR-FIN-01 «العمليات» (328:12108) | Sources: Figma shows channels (direct sale, each institute, recorded course). Programs and bids/contracts have no revenue rows yet. | Channels from real ledger rows of the current month (direct and recorded always listed, institutes when they paid). Insights and the «قفزة …» tip appear only when the data supports them. «صدّر …» downloads CSV (opens in Excel); «نزّل كشف التسوية / الإيصال» prints the page (save as PDF). |
| W2-FIN-9 | Sequences drawn left-to-right (FIN-01 empty «كيف تسير الخطوات؟», FIN-02 «الإجمالي · العمولة · الصافي», tabs) | Same as T24. | Shown in RTL reading order. The income chart keeps Figma's placement (current month at the right). |
| W2-FIN-10 | «اعترض على التسوية» | No dispute screen for trainers. | Opens `/messages?f=support` (like T9) during the 30 days after the settlement closes. |
| W2-FIN-11 | Commission settings | `trainer_commission_percent` (Wave 1 dashboard) and `platform_commission_percent` (programs/courses) both existed. | One source: `platform_commission_percent()`, frozen on each ledger row at sale time. `trainer_commission_percent` is no longer read. |
