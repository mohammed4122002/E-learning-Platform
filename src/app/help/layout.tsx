...
export default async function PublicHelpLayout({ children }: LayoutProps<"/help">) {
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-bg-surface">
      ...
      <header className="sticky top-0 z-20 flex h-[76px] w-full items-center gap-2 border-b border-border-divider bg-bg-surface px-4 sm:gap-3 sm:px-8">
        <Link href={user ? homePathFor(user) : "/login"} className="flex min-w-0 items-center gap-3 rounded-12 focus-ring">
          ...
          <span className="flex min-w-0 flex-col">
            <span className="truncate type-title text-text-primary">بوابة التدريب</span>
            <span className="hidden type-caption text-text-muted sm:block">مركز المساعدة</span>
          </span>
        </Link>
      ...
      <main id="main" tabIndex={-1} className="mx-auto flex w-full max-w-[1160px] flex-1 flex-col gap-6 px-4 pt-8 pb-14 outline-none sm:px-6 lg:px-12">
        <h1 className="sr-only">مركز المساعدة — بوابة التدريب</h1>
        {children}
      </main>
    </div>
  );
